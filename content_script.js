  ...
  function removeWithFade(el) {
    try {
      requestAnimationFrame(() => {
        try {
          el.classList.add('openwhen-fade');
        } catch (e) {}
      });
      try {
        el.addEventListener(
  ...
  function showBanner(text, source, missedAt, scheduleId) {
    try {
      ...
      if (rawMessage) {
        ...
        headline.className = 'openwhen-headline';
        headline.textContent = rawMessage;
        messageStack.appendChild(headline);
        if (rawSource && rawSource !== rawMessage) {
          const urlLine = document.createElement('div');
          urlLine.className = 'openwhen-url';
          urlLine.textContent = rawSource;
          messageStack.appendChild(urlLine);
        }
      } else {
      ...
      if (missedAt) {
        try {
          const d = new Date(Number(missedAt));
          if (!isNaN(d.getTime())) {
            const when = document.createElement('div');
            when.className = 'openwhen-when';
            when.textContent = `scheduled for: ${d.toLocaleString()}`;
      ...
      if (data && data.type === 'openwhen_cancel_response' && data.id) {
        try {
          ...
          if (t) {
            try {
              const cb =
                t.querySelector && t.querySelector('.openwhen-cancel-btn');
              if (cb) cb.remove();
            } catch (e) {}
          }
          ...
          if (b) {
            try {
              const cb2 =
                b.querySelector && b.querySelector('.openwhen-cancel-btn');
              if (cb2) cb2.remove();
            } catch (e) {}
          }
          ...
          try {
            ...
            if (el) {
              ...
              toast.className = 'openwhen-cancel-toast';
              toast.textContent = 'Schedule cancelled';
              el.appendChild(toast);
              setTimeout(() => {
                try {
                  requestAnimationFrame(() => {
                    toast.classList.add('openwhen-toast-fade');
                  });
                  setTimeout(() => {
                    try {
                      toast.remove();
