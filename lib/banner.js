function showBanner(text, source, missedAt, scheduleId) {
  ...
  headline.textContent = `opened by OpenWhen (${source || 'scheduled'})`;
  const msg = document.createElement('div');
  msg.className = 'openwhen-url';
    ...
    try {
      ...
      const showCancelToast = (msg) => {
        try {
          ...
          t.className = 'openwhen-cancel-toast';
          t.textContent = msg || 'Schedule cancelled';
          content.appendChild(t);
          setTimeout(() => {
            try {
              requestAnimationFrame(() => {
                t.classList.add('openwhen-toast-fade');
              });
              setTimeout(() => {
                try {
                  t.remove();
      ...
      cancelBtn.addEventListener('click', () => {
        try {
          ...
          const handleResp = (ok) => {
            ...
            if (ok) {
              try {
                showCancelToast('Schedule cancelled');
              } catch (e) {}
              try {
                cancelBtn.remove();
              } catch (e) {}
                ...
                function tryOnce() {
                  try {
                    ...
                    if (attempt < attempts) {
                      setTimeout(
                        tryOnce,
                        delays[Math.min(attempt - 1, delays.length - 1)] || 100
                      );
                    } else {
                      cb(null, err);
                    }
