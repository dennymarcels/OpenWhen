...
### Added
- **Edit from banner**: An "edit" button is now shown in the in-page reminder banner, allowing users to open the options page pre-populated with the schedule for that page directly from the banner.
- **Delete from banner with undo**: The "cancel schedule" button in the banner has been replaced by a "delete" button that triggers a timed undo countdown (approx. 5 seconds) before the schedule is permanently removed, preventing accidental deletions.
- **Multiple daily trigger times**: Daily schedules now support multiple trigger times per day. Users can add or remove time entries via the "add time" button in the options, popup, and sidebar forms.
- **Multiple daily times in schedule display**: The schedule list and notifications now show all configured times for daily schedules (e.g. `daily @ 09:00, 12:00, 18:00`).

### Changed
- Banner action buttons ("edit" and "delete") are now grouped in a dedicated `.openwhen-actions` container for a cleaner layout.
- The "cancel schedule" label has been renamed to "delete" to better reflect the destructive action; the undo flow replaces both action buttons with a dismissible countdown badge.
- `lib/scheduler.js` reformatted for readability (no logic changes); `computeNextForSchedule` and `occurrencesBetween` now fully support the `times` array for daily schedules.
- `.gitignore` updated to exclude the `.history/` folder generated by the VS Code Local History extension.

### Fixed
- `occurrencesBetween` for daily schedules now correctly iterates over all configured times instead of only the first one, fixing missed-occurrence detection when multiple daily times are set.
- The undo flow in the banner prevents the schedule from being deleted if the user clicks "undo" within the countdown window.
