      ...
      chrome.storage.local.get(['openwhen_prefill_url'], res => {
        ...
        try{
          chrome.tabs && chrome.tabs.query && chrome.tabs.query({active:true,lastFocusedWindow:true}, tabs => {
            ...
            if(v){ const input = document.getElementById('url'); if(input) input.value = v; }
          });
        }catch(e){}
      });
    }
